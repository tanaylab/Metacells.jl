...
function do_compute_metacells_genes_approximated_linear_covered_fractions!(
    ...
    @threads :greedy for block_index in 1:n_blocks
        ...
        coefficient_per_covered_gene_per_used_module =
            daf["/ gene & is_covered / module & is_used ; block = $(block_name) : $(block_name)_covered_coefficient"].array

        @views base_covered_fraction_per_used_module_of_block =
            daf["/ module & is_used ; block = $(block_name) / block = $(block_name) : base_covered_fraction"].array

        @views base_covered_fraction_per_covered_gene_of_block =
            daf["/ gene & is_covered / block = $(block_name) : base_covered_fraction"].array

        @views linear_covered_fraction_per_used_module_per_block_metacell =
            daf["/ module & is_used ; block = $(block_name) / metacell & block = $(block_name) : $(covered_fraction)"].array

        offset_linear_covered_fraction_per_used_module_per_block_metacell =
            linear_covered_fraction_per_used_module_per_block_metacell .- base_covered_fraction_per_used_module_of_block
...
@logged @computation Contract(
    ...
    @threads :greedy for block_index in 1:n_blocks
        ...
        offset_covered_fraction_per_neighborhood_metacell_per_used_module =
            Matrix{Float32}(undef, n_neighborhood_metacells, n_used_modules)
        for (used_module_position, used_module_index) in enumerate(used_module_indices)
            ...
            offset_covered_fraction_per_neighborhood_metacell_per_used_module[:, used_module_position] .=
        ...
        coefficient_per_used_module_per_covered =
            coefficient_per_module_per_covered = coefficient_per_module_per_covered[used_module_indices, :]

        predicted_covered_fraction_per_covered_per_neighborhood_metacell = max.(
            (
                transpose(coefficient_per_used_module_per_covered) *
                transpose(offset_covered_fraction_per_neighborhood_metacell_per_used_module)
            ) .+ base_covered_fraction_per_covered,
            0.0,
        )
        ...
        RMSE_per_block[block_index] = sqrt(mean((predicted_covered_fraction_per_covered_per_neighborhood_metacell .- estimated_covered_fraction_per_covered_per_neighborhood_metacell) .^ 2))
    end
