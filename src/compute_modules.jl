...
@logged @computation Contract(
    ...
    data = [
        ...
        block_gene_is_environment_marker(RequiredInput),
        block_block_is_in_environment_matrix(RequiredInput),
        block_gene_module_index_matrix(GuaranteedOutput),
    ...
    add_axis!(daf, "module", "GM" .* string.(collect(1:n_modules)))

    module_index_per_gene_per_block = zeros(UInt32, n_gene, n_blocks)
    ...
    for (block_index, gene_indices_per_module_of_block) in enumerate(gene_indices_per_module_per_block)
        for (module_index, gene_indices_of_module_of_block) in enumerate(gene_indices_per_module_of_block)
            module_index_per_gene_per_block[gene_indices_of_module_of_block, block_index] .= module_index
    ...
    set_matrix!(daf, "gene", "block", "module_index", bestify(module_index_per_gene_per_block))  # NOJET
