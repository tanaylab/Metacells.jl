...
import Metacells.Contracts.block_gene_is_environment_marker
...
import Metacells.Contracts.block_modules_RMSE_vector
import Metacells.Contracts.block_modules_XRMSE_vector
...
@logged @computation Contract(;
    ...
    module_index_per_gene_per_block = get_matrix(daf, "gene", "block", "module_index")
...
For each block, compute a local linear model for its environment, based on the used gene modules. This is a simple
...
@logged @computation Contract(
    ...
    data = [
        ...
        block_modules_RMSE_vector(GuaranteedOutput),
        block_modules_XRMSE_vector(GuaranteedOutput),
    ],
) function compute_approximation!(
    daf::DafWriter;
    ...
    modules_RMSE_per_block = Vector{Float32}(undef, n_blocks)
    modules_XRMSE_per_block = Vector{Float32}(undef, n_blocks)
    ...
    parallel_loop_with_rng(1:n_blocks; rng) do block_index, rng
        ...
        n_used_modules, rmse, xrmse = with_reusable(reusable_storage) do reusable_matrices
        ...
        modules_RMSE_per_block[block_index] = rmse
        modules_XRMSE_per_block[block_index] = xrmse

        counter = atomic_add!(progress_counter, 1)
        @debug (
            "- Environment: $(block_name) ($(percent(counter + 1, n_blocks)))" *
            " Gene Modules: $(n_used_modules)" *
            " RMSE: $(rmse)" *
            " XRMSE: $(xrmse)"
        )
    ...
    @debug "Mean RMSE: $(mean(modules_RMSE_per_block)) XRMSE: $(mean(modules_XRMSE_per_block))"
    set_vector!(daf, "block", "modules_RMSE", modules_RMSE_per_block; overwrite)
    set_vector!(daf, "block", "modules_XRMSE", modules_XRMSE_per_block; overwrite)
    return nothing
end
...
function compute_block_approximation(
    ...
    is_used_per_module = daf["/ block = $(block_name) / module : is_used"].array
    used_module_indices = findall(is_used_per_module)
    n_used_modules = length(used_module_indices)
    @assert n_used_modules > 0
    ...
    rmse, coefficient_per_used_module_per_covered = compute_environment_model(
        daf;
        block_name,
        covered_fraction_per_covered_per_metacell,
        used_module_indices,
    ...
    coefficient_per_module_per_gene[used_module_indices, covered_indices] .= coefficient_per_used_module_per_covered
    set_matrix!(
        daf,
        "module",
        "gene",
    ...
    return (n_used_modules, rmse, xrmse)
end
...
function compute_environment_model(
    ...
    block_name::AbstractString,
    covered_fraction_per_covered_per_metacell::AbstractMatrix{<:AbstractFloat},
    used_module_indices::AbstractVector{<:Integer},
    ...
    module_index_per_covered = daf["/ block = $(block_name) / gene & is_covered : module_index"].array
    ...
    offset_covered_fraction_per_train_metacell_per_used_module =
        Matrix{Float32}(undef, n_train_metacells, n_used_modules)
    if train_metacell_indices === test_metacell_indices
        offset_covered_fraction_per_test_metacell_per_used_module =
            offset_covered_fraction_per_train_metacell_per_used_module
        offset_covered_fraction_per_covered_per_test_metacell = offset_covered_fraction_per_covered_per_train_metacell
    else
        offset_covered_fraction_per_test_metacell_per_used_module =
            Matrix{Float32}(undef, n_test_metacells, n_used_modules)
    ...
    for (used_module_position, used_module_index) in enumerate(used_module_indices)
        covered_module_genes_positions = findall(module_index_per_covered .== used_module_index)
        ...
        offset_covered_fraction_per_train_metacell_per_used_module[:, used_module_position] .=
            vec(sum(covered_fraction_per_covered_module_gene_per_train_metacell; dims = 1)) .-
            base_covered_fraction_per_module[used_module_index]

        if offset_covered_fraction_per_test_metacell_per_used_module !==
           offset_covered_fraction_per_train_metacell_per_used_module
            ...
            offset_covered_fraction_per_test_metacell_per_used_module[:, used_module_position] .=
                vec(sum(covered_fraction_per_covered_module_gene_per_test_metacell; dims = 1)) .-
                base_covered_fraction_per_module[used_module_index]
        end
    end

    coefficient_per_used_module_per_covered =
        offset_covered_fraction_per_train_metacell_per_used_module \
        transpose(offset_covered_fraction_per_covered_per_train_metacell)
    @assert_matrix(coefficient_per_used_module_per_covered, n_used_modules, n_covered_genes)

    predicted_covered_fraction_per_covered_per_test_metacell = max.(
        (
            transpose(coefficient_per_used_module_per_covered) *
            transpose(offset_covered_fraction_per_test_metacell_per_used_module)
    ...
    return (rmse, coefficient_per_used_module_per_covered)
end
